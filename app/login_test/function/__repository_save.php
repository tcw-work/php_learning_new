        ...
        if (isset($_SESSION['login'])) {//ログアウト情報がセットされていれば
            ...
            foreach ($_POST['save_items'] as $item) {//foreach(変数, 代入される配列変数)でname="save_items[]に設定した配列の要素を一つずつ順番に取り出す
                ...
                if ($count_result >= 20) {//カラムの合計が20以上の場合
                    ...
                    $update_query = "UPDATE favorites SET goods = goods + 1 WHERE user_id = '$user_id' AND favorite_id = '$favorite_id'";//保存処理が走るたびに、そのuser_idを持つfavorite_idをインクリメント
